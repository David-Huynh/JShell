				...
				{ // separate path into its directories
					...
					if (currDir == null) {
						...
						if (path.get(path.size() - 1).equals("..")
							...
							directoryIndex = currDir
									.isSubDir(path.get(path.size() - 1));
							fileIndex = currDir
									.containsFile(path.get(path.size() - 1));
							...
							if (directoryIndex != -1) { // check if parameter is
								...
								shell.println(parameters[i] + ":");
								list(shell,
										((Directory) currDir.getDirContents()
												.get(directoryIndex))
														.getDirContents());
							} else if (fileIndex != -1) {
