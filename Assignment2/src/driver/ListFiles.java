...
public class ListFiles extends ShellCommand {
  ...
  public static void performOutcome(JShell shell, String[] parameters) {
    ...
    if (parameters.length == 1) {// Case 1: Called with no parameters, list files of current
                                 // directory
      list(shell, fileList);
    } else {
      for (int i = 1; i < parameters.length; i++) {// Case 2: Called with >= 1 parameters, index
        ...
        { 
          ...
          if (currDir == null) {
            ...
            if (path.get(path.size() - 1).equals("..") || path.get(path.size() - 1).equals(".")) {
              ...
              if (directoryIndex != -1) {
                // check if parameter is a directory in
                // current directory
                shell.println(parameters[i] + ":");
                list(shell,
