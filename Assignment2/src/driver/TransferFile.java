...
public class TransferFile {
	 ...
	 */
	public static boolean validateNumberOfParameters(JShellInterface shell, String operation, int paramLength) {
		if (paramLength != 3) {
	...
	/**
	 * Checks whether toMove is an ancestor of moveHere, and prints an error if it
	 * is
	 * 
	 ...
	 */
	public static boolean validateParents(Directory toMove, Directory moveHere, JShellInterface shell,
			String[] parameters) {
		if (toMove == null || moveHere == null | toMove.checkParents(moveHere, shell)) {
	...
	/**
	 * Checks if the name of toMove is equal to any of the names of the StorageUnits
	 * in moveHere, and prints an error if it is
	 * 
	 * @param toMove     The StorageUnit that has the name we want to check
	 *                   uniqueness for
	 ...
	 *         moveHere
	 */
	public static boolean validateNames(StorageUnit toMove, Directory moveHere, JShellInterface shell,
			String[] parameters) {
		if (toMove == null || moveHere == null || toMove.checkIdenticalNames(moveHere)) {
			PrintError.reportError(shell, parameters[0],
					"Cannot move/copy '" + parameters[1] + "' into '" + parameters[2]
							+ "' since a file/directory with the same " + "name already exists in '" + parameters[2]
							+ "'");
			return false;
		}
		return true;
	 ...
	 */
	public static boolean validateFileToFile(StorageUnit toMove, JShellInterface shell, String[] parameters) {
		if (toMove != null && toMove.isFile()) {
			return true;
		} else {
			PrintError.reportError(shell, parameters[0],
					"Cannot overwrite non-directory '" + parameters[2] + "' with directory '" + parameters[1] + "'");
			return false;
		}
	}

	/**
	 * Checks whether dest is a new file path or a new directory path and returns a
	 * StorageUnit with same name as the element at the end of the path
	 * 
	...
	public static StorageUnit determineNewStorageUnit(String dest) {
		...
		if (moveHerePath.getPathElements().length > 0) {
			newStorageUnitName = moveHerePath.getPathElements()[moveHerePath.getPathElements().length - 1];
		} else {
			return null;
		}
		if (dest.endsWith("/")) {
