...
/**
 * A StorageUnit stores data and lives in a JShell's storage system. Subclasses of StorageUnit are
 * File and Directory.
 */
...
abstract public class StorageUnit implements Serializable {
  ...
  public Path getFullPath(JShell shell) {
    ...
    if (currDir != shell.getRootDir()) {
      pathString =
          currDir.getParentDir().name + "/" + currDir.name + pathString;
   ...
   * 
   * @param parent The new location of the StorageUnit
   */
  ...
  /**
   * Checks if a string has any of the characters not allowed in the name of a StorageUnit
   * 
   * @param filename The string to be checked for forbidden characters
