      ...
      {
        {
          ...
          if (parameters[i].startsWith("/")) { // path is absolute
            if (!parameters[i].equals("/") && path.get(1).equals("root")) { // check if first
                                                                            // directory in abs path
                                                                            // is root
              currDir = shell.getRootDir();
              path.remove(0); // remove the empty string and the root directory
              path.remove(0); // treat as if path was relative to root
            } else {
          ...
          currDir = Directory.cycleDir(path, 0, currDir);
